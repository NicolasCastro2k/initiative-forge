...
gamesRouter.get("/", async (req, res) => {
  try {
    ...
    const games = memberships.map((membership) => {
      // CORRECCIÓN: Garantizar que la lista de miembros refleje el rol real del creador
      if (membership.game.members) {
        membership.game.members = membership.game.members.map(m => ({
          ...m,
          role: m.userId === membership.game.dmUserId ? "DM" : m.role
        }));
      }

      return {
        ...membership.game,
        role: (membership.game.dmUserId === user.id ? "DM" : membership.role) as any,
      };
    });
...
gamesRouter.post("/join", async (req, res) => {
  try {
    ...
    const membership = await prisma.gameMember.upsert({
      ...
      update: game.dmUserId === user.id ? { role: "DM" } : {},
      create: {
        gameId: game.id,
        userId: user.id,
        role: (game.dmUserId === user.id ? "DM" : "PLAYER") as any,
      },
    });
...
gamesRouter.get("/:gameId/characters", async (req, res) => {
  try {
    ...
    if (!membership && (!game || game.dmUserId !== user.id)) {
      return res.status(404).json({
    ...
    const role = (game?.dmUserId === user.id ? "DM" : (membership?.role ?? "PLAYER")) as any;
...
gamesRouter.delete("/:gameId/characters/:selectionId", async (req, res) => {
  try {
    ...
    const membership = await getUserMembership(gameId, user.id);
    const game = await prisma.game.findUnique({ where: { id: gameId }, select: { dmUserId: true } });
    ...
    const isDm = game?.dmUserId === user.id || membership?.role === "DM";
...
gamesRouter.get("/:gameId", async (req, res) => {
  try {
    ...
    const gameId = String(req.params.gameId);
    let membership = await getUserMembership(gameId, user.id);
    ...
    // AUTO-HEAL: Si eres el dueño del juego, forzamos y reparamos tu rol en la base de datos
    if (game.dmUserId === user.id) {
      if (!membership || membership.role !== "DM") {
        membership = await prisma.gameMember.upsert({
          where: { gameId_userId: { gameId, userId: user.id } },
          update: { role: "DM" },
          create: { gameId, userId: user.id, role: "DM" }
        });
      }
    }

    // CORRECCIÓN: Garantizar que la lista interna de miembros devuelta al frontend refleje el rol forzado.
    // Muchos frontends leen `game.members` en lugar de la propiedad `role` del root.
    game.members = game.members.map(member => ({
      ...member,
      role: member.userId === game.dmUserId ? "DM" : member.role
    }));
